      ...
      <form action="{{ route('teacherlessonplanstore')}}" method="POST" class="w-full max-w-lg" enctype="multipart/form-data">
        ...
        <div class="flex flex-wrap -mx-3 mb-6">
          <div class="w-full px-3">
            ...
            <select name="type" id="type" class="appearance-none block w-full bg-gray-200 text-gray-700 border border-gray-200 rounded py-3 px-4 mb-3 leading-tight focus:outline-none focus:bg-white focus:border-gray-500">
              <option>Pilih Tipe Pembelajaran</option>
          ...
          <div class="w-full px-3">
            ...
            <select name="teaching_hour_id" id="teaching_hour_id" class="appearance-none block w-full bg-gray-200 text-gray-700 border border-gray-200 rounded py-3 px-4 mb-3 leading-tight focus:outline-none focus:bg-white focus:border-gray-500">
              <option>Pilih Mata Pelajaran</option>
              @foreach($datas as $data)
              <option value="{{$data->id}}">{{$data->subject->name}} - {{$data->classmodel->name}}</option>
              @endforeach
            </select>
          </div>
        </div>
        <div class="md:flex md:items-center">
          <div class="md:w-1/3">
            ...
            <button onclick="window.location='{{ url ('/teacher/lessonplan') }}'" class="shadow bg-red-600 hover:bg-red-400 focus:shadow-outline focus:outline-none text-white font-bold py-2 px-4 rounded" type="button">
              Cancel
            </button>
          </div>
