      ...
      <div class="w-full overflow-x-auto">
        <table class="w-full">
          <tbody class="bg-white">
            ...
            <tr class="text-gray-700 text-center">
              <td class="px-4 py-3 text-ms border font-semibold">Hasil Belajar Keseluruhan</td>
              <td class="px-4 py-3 text-ms border font-semibold">:</td>
              <td class="px-4 py-3 text-ms border">
                @if($check > 0)
                <a href="/teacher/studentscore/print/{{$data->id}}">
                  <span class="inline-block rounded-full text-white bg-green-500 px-2 py-1 text-xs font-bold mr-3">Lihat</span>
                </a>
                @else
                <a href="/teacher/studentscore/generate/{{$data->id}}">
                  <span class="inline-block rounded-full text-white bg-yellow-500 px-2 py-1 text-xs font-bold mr-3">Klik Untuk Generate</span>
                </a>
                @endif
              </td>
            </tr>
        ...
        <table class="w-full">
          <thead>
            <tr class="text-md text-center font-semibold tracking-wide text-left text-gray-900 bg-gray-100 uppercase border-b border-gray-600">
              <th class="px-4 py-3" colspan="7">Siswa Mata Pelajaran</th>
            </tr>
            <tr class="text-md text-center font-semibold tracking-wide text-left text-gray-900 bg-gray-100 uppercase border-b border-gray-600">
          ...
          <tbody class="bg-white">
            ...
            <tr class="text-gray-700 text-center">
              <td class="px-4 py-3 text-ms border font-semibold">{{$student->user->studentPersonalData->name}}</td>
              <td class="px-4 py-3 text-ms border font-semibold">{{$student->user->studentPersonalData->student_number}} / {{$student->user->studentPersonalData->national_student_number}}</td>
              <td class="px-4 py-3 text-ms border font-semibold">
                <a href="{{ url ('/teacher/studentscore/show/student', array("$student->id", "$data->id")) }}" class="text-green-600 hover:text-green-400 mr-2">
